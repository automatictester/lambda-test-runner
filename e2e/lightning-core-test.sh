...
RESPONSE_JSON=lightning-core-response.json
rm -f ${RESPONSE_JSON}

START=`date +%s`
aws lambda invoke --function-name LambdaTestRunner --region eu-west-2 --cli-read-timeout 0 --payload file://lightning-core-payload.json ${RESPONSE_JSON}
END=`date +%s`
EXEC_TIME=$((END-START))
