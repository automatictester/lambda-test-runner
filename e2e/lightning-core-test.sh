...
START=`date +%s`
aws lambda invoke --function-name LambdaTestRunner --region eu-west-2 --invocation-type RequestResponse --payload file://lightning-core-payload.json lightning-core-response.json
END=`date +%s`
EXEC_TIME=$((END-START))
